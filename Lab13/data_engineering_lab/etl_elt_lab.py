...
# ── Spark Session ──────────────────────────────────────────── 
spark = SparkSession.builder.appName("ETL_vs_ELT_Lab").master("local[*]").getOrCreate() 
...
transformed_df.write.mode("overwrite").parquet(ETL_PATH) 
...
raw_df.write.mode("overwrite").parquet(ELT_RAW) 
